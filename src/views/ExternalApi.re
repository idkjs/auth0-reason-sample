...
let make = () => {
  ...
  // }
  let callApi = () => {
    let token = getAccessTokenSilently(Some(options));
    Fetch.fetchWithInit(
      url,
      Fetch.RequestInit.make(
        ~method_=Get,
        ...
        {state.error == Some("consent_required")
           ? <Alert color="warning">
               ...
               <a
                 ...
                 onClick={e => handle(e, handleConsent)->ignore}>
        ...
        {state.error == Some("login_required")
           ? <Alert color="warning">
               ...
               <a
                 href="#/"
                 className="alert-link"
                 onClick={e => handle(e, handleLoginAgain)->ignore}>
        ...
        {audience == None
           ? <Alert color="warning">
               <p>
                ...
                out more information on"->React.string
           ...
           : React.null}
        <Button
          color="primary" className="mt-5" onClick=callApi disabled={audience == None}>
